...
const JobsViewHeader = () => {
  ...
  const { specificJob, getRandomColor,sendJobNotification, convertToLPA ,user,verifyJob,getJobsByIdByAdmin,isLoading} = useAppContext();
  const handleVerify=()=>{
    if(window.confirm(`Are you sure you want to ${specificJob?.verified===true?"unverify":"verify"} this job?`)){
    ...
    <div className="shadow bg-white w-full rounded-xl flex flex-col">
      ...
      <div className="flex flex-col relative w-full">
        ...
        <div className="flex flex-row absolute top-0 right-0 mt-4 mr-4">
          <button className="border bg-white px-4 py-2 rounded-lg mr-2 text-sm font-medium" onClick={handleVerify} >
            {specificJob?.verified===true?<FaTimes title="Unverify Job"/>:<FaCheckCircle title="Verify Job"/>}
      ...
      >
        ...
        <div className="flex flex-col absolute md:mt-6 mt-8 ml-4">
          <div className="md:w-32 md:h-32 w-24 h-24 rounded-full m-4 bg-gray-300 p-1 border ">
           ...
           {specificJob?.verified===true ? <FaCheckCircle className=" rounded-full md:text-3xl text-3xl font-bold bg-blue-500 text-white" title="Verified"/>:
            <FaTimes className=" rounded-full md:text-3xl text-xl font-bold bg-red-500 text-white" title="Unverified"/>}
            </div>
          </div>
